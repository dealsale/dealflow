...
export function MLeads({ df }: { df: DealFlowState }) {
  return (
    <section data-screen-label="Móvil Leads">
      ...
      <div style={{ background: 'var(--df-surface)', border: '1px solid var(--df-border)', borderRadius: 14, overflow: 'hidden' }}>
        {df.leads.map((l) => (
          <div key={l.id} style={{ display: 'flex', gap: 11, padding: '13px 14px', borderBottom: '1px solid var(--df-border)' }}>
            <div style={l.avatarStyle}>{l.iniciales}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'baseline' }}>
                <span style={{ fontWeight: 600, fontSize: 14 }}>{l.nombre}</span>
                <span style={{ color: 'var(--df-text-faint)', fontSize: 11, marginLeft: 'auto' }}>{l.hora}</span>
              </div>
