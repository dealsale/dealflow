...
function PruebaAsistente({ storeId }: { storeId: string }) {
  ...
  return (
    <div style={{ background: 'var(--df-surface)', border: '1px solid var(--df-border)', borderRadius: 12, boxShadow: '0 1px 2px rgba(15,23,42,.04)', marginTop: 14, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 20px', borderBottom: '1px solid var(--df-border)' }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 15 }}>🧪 Prueba tu asistente</div>
          <div style={{ color: 'var(--df-text-muted)', fontSize: 13 }}>Chatea aquí mismo con tu IA, como si fueras un cliente. No usa WhatsApp: funciona aunque Meta esté caído.</div>
        </div>
        <div style={{ flex: 1 }} />
        <button
          onClick={reiniciar}
          title="Empieza una conversación nueva desde cero"
          style={{ background: 'var(--df-surface)', color: 'var(--df-text-muted)', border: '1px solid var(--df-border)', borderRadius: 8, padding: '7px 12px', fontFamily: 'inherit', fontWeight: 600, fontSize: 12.5, cursor: 'pointer', whiteSpace: 'nowrap' }}
        >
          ↻ Empezar de nuevo
        </button>
      </div>

      <div ref={scrollRef} style={{ height: 340, overflowY: 'auto', background: 'var(--df-bg)', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {mensajes.length === 0 && (
          <div style={{ margin: 'auto', textAlign: 'center', color: 'var(--df-text-faint)', fontSize: 13.5, lineHeight: 1.7 }}>
        ...
        {mensajes.map((m, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', justifyContent: mio ? 'flex-end' : 'flex-start' }}>
              <div style={{ maxWidth: '72%', background: mio ? 'var(--df-green-subtle)' : 'var(--df-surface)', border: '1px solid ' + (mio ? '#BBF7D0' : 'var(--df-border)'), borderRadius: 12, padding: '9px 12px', fontSize: 13.5, lineHeight: 1.5 }}>
                ...
                <span style={{ display: 'block', textAlign: 'right', color: 'var(--df-text-faint)', fontSize: 10.5, marginTop: 3 }}>{m.hora}</span>
              </div>
            </div>
          );
        })}
        {enviando && <div style={{ color: 'var(--df-text-faint)', fontSize: 12.5 }}>El asistente está escribiendo…</div>}
      </div>

      <div style={{ display: 'flex', gap: 8, padding: 12, borderTop: '1px solid var(--df-border)' }}>
        <input
          className="df-input"
          value={texto}
          ...
          style={{ flex: 1, border: '1px solid var(--df-border)', borderRadius: 10, padding: '11px 13px', fontFamily: 'inherit', fontSize: 13.5, minHeight: 44, boxSizing: 'border-box' }}
        />
        <button
          onClick={enviar}
      ...
      <div className="df-collapse" style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 14, alignItems: 'start' }}>
        <div style={{ background: 'var(--df-surface)', border: '1px solid var(--df-border)', borderRadius: 12, padding: 20, boxShadow: '0 1px 2px rgba(15,23,42,.04)' }}>
          ...
          <textarea
            ...
            rows={9}
            style={{ width: '100%', boxSizing: 'border-box', border: '1px solid var(--df-border)', borderRadius: 8, padding: 12, fontFamily: 'inherit', fontSize: 14, lineHeight: 1.6, color: 'var(--df-text-strong)', resize: 'vertical' }}
          />
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 12 }}>
            ...
            {df.assistantSaved && <span style={{ color: 'var(--df-brand)', fontSize: 13, fontWeight: 600 }}>✓ Guardado. El asistente ya vende así.</span>}
          </div>
        </div>

        <div style={{ background: 'var(--df-surface)', border: '1px solid var(--df-border)', borderRadius: 12, padding: 20, boxShadow: '0 1px 2px rgba(15,23,42,.04)' }}>
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 4 }}>Reglas</div>
          <div style={{ color: 'var(--df-text-muted)', fontSize: 13, marginBottom: 12 }}>Se cumplen siempre, pase lo que pase.</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--df-text-faint)', fontWeight: 600 }}>{df.rules.length} regla{df.rules.length === 1 ? '' : 's'}</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14, maxHeight: 340, overflowY: 'auto', paddingRight: 4 }}>
            {df.rules.length === 0 && <div style={{ color: 'var(--df-text-faint)', fontSize: 13, padding: '8px 2px' }}>Aún no hay reglas. Agrega la primera abajo.</div>}
            {df.rules.map((r, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: 'var(--df-bg)', border: '1px solid var(--df-border)', borderRadius: 8, padding: '10px 12px' }}>
                <span style={{ color: 'var(--df-text-faint)', fontWeight: 700, flexShrink: 0, fontSize: 12, minWidth: 18, marginTop: 6 }}>{i + 1}</span>
                <textarea
                  value={r.texto}
                  onChange={(e) => r.editar(e.target.value)}
                  rows={1}
                  className="df-input"
                  style={{ flex: 1, fontSize: 13, lineHeight: 1.5, border: '1px solid transparent', background: 'transparent', borderRadius: 6, padding: '4px 6px', fontFamily: 'inherit', resize: 'vertical', color: 'var(--df-text-strong)' }}
                  onFocus={(e) => { e.currentTarget.style.background = 'var(--df-surface)'; e.currentTarget.style.borderColor = 'var(--df-border)'; }}
                  onBlur={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'transparent'; }}
                />
                <span onClick={r.remove} className="df-danger-hover" style={{ color: 'var(--df-text-faint)', cursor: 'pointer', fontSize: 14, lineHeight: 1, padding: 2, marginTop: 6 }}>
                  ✕
                </span>
              </div>
            ))}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              style={{ flex: 1, border: '1px solid var(--df-border)', borderRadius: 8, padding: '10px 12px', fontFamily: 'inherit', fontSize: 13 }}
            />
            <button
              onClick={df.addRule}
