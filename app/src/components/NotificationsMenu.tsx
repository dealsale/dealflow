...
function Switch({ on, onClick, disabled }: { on: boolean; onClick: () => void; disabled?: boolean }) {
  return (
    <span
      ...
      style={{ width: 38, height: 22, borderRadius: 999, background: on ? '#059669' : '#CBD5E1', position: 'relative', cursor: disabled ? 'default' : 'pointer', flexShrink: 0, opacity: disabled ? 0.5 : 1, transition: 'background .15s' }}
    >
      <span style={{ position: 'absolute', top: 2, left: on ? 18 : 2, width: 18, height: 18, borderRadius: '50%', background: '#fff', transition: 'left .15s', boxShadow: '0 1px 2px rgba(15,23,42,.3)' }} />
    </span>
  );
}
  ...
  const fila = (titulo: string, sub: string, on: boolean, onClick: () => void, disabled?: boolean) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 4px' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13.5, fontWeight: 600, color: '#1E293B' }}>{titulo}</div>
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <span
        ...
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, borderRadius: 8, cursor: 'pointer', color: activo ? '#059669' : '#64748B', position: 'relative' }}
      >
        <IconBell muted={!activo} />
        {activo && <span style={{ position: 'absolute', top: 5, right: 6, width: 7, height: 7, borderRadius: '50%', background: '#10B981', border: '1.5px solid #fff' }} />}
      </span>

      {open && (
        <div style={{ position: 'absolute', top: '130%', right: 0, width: 288, background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, boxShadow: '0 20px 50px -15px rgba(15,23,42,.3)', padding: 14, zIndex: 60 }}>
          <div style={{ fontWeight: 800, fontSize: 14.5, marginBottom: 2 }}>Notificaciones</div>
          <div style={{ color: '#94A3B8', fontSize: 12, marginBottom: 8 }}>Avisos en este dispositivo (web y app).</div>
          ...
          {bloqueado && (
            <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', color: '#B91C1C', borderRadius: 8, padding: '8px 10px', fontSize: 12, lineHeight: 1.45, margin: '2px 0 6px' }}>
              Están bloqueadas en el navegador. Ábrelas desde el candado 🔒 de la barra de direcciones y vuelve a activarlas.
            </div>
          )}

          <div style={{ borderTop: '1px solid #F1F5F9', margin: '4px 0' }} />
          <div style={{ fontSize: 11, fontWeight: 700, color: '#94A3B8', letterSpacing: '.04em', textTransform: 'uppercase', padding: '6px 4px 2px' }}>Qué avisar</div>
